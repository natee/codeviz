...
| `vitest`              | ^4.0.16  | 单元测试框架（Vitest）                  | 🟢 核心 |
| `@vitest/coverage-v8` | ^4.0.16  | Vitest 代码覆盖率工具                   | 🟡 重要 |
| `@vitest/ui`          | ^4.0.16  | Vitest 浏览器 UI 界面                  | 🟢 辅助 |
...
3. **测试验证**
   ```bash
   pnpm test          # 运行单元测试
   pnpm run test:watch # 监听模式开发
   pnpm run test:ui    # 浏览器 UI 界面
   pnpm run test:coverage # 检查覆盖率
   pnpm run build     # 构建验证
   ```
